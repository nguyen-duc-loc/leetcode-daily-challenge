   ...
   public:
    int maximumScore(vector<int>& nums, int k) {
        ...
        for (const auto& [num, i] : numAndIndexes) {
            // nums[i] is the maximum value in the range [left[i] + 1, right[i]
            // - 1] So, there are (i - left[i]) * (right[i] - 1) ranges where
            // nums[i] will be chosen.
            const long rangeCount =
                static_cast<long>(i - left[i]) * (right[i] - i);
