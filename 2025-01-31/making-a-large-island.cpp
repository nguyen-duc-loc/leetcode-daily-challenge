class Solution {
   public:
    int largestIsland(vector<vector<int>>& grid) {
        ...
        int maxSize = 0;
        // sizes[i] := the size of the i-th connected component (starting from
        // 2)
        vector<int> sizes{0, 0};

        // For each 1 in the grid, paint all the connected 1s with the next
        // available color (2, 3, and so on). Also, remember the size of the
        // island we just painted with that color.
        for (int i = 0; i < m; ++i)
            for (int j = 0; j < n; ++j)
                if (grid[i][j] == 1)
                    sizes.push_back(
                        paint(grid, i, j, sizes.size()));  // Paint 2, 3, ...
   ...
   private:
    int paint(vector<vector<int>>& grid, int i, int j, int id) {
        ...
        if (grid[i][j] != 1)
            return 0;
        grid[i][j] =
            id;  // grid[i][j] is part of the id-th connected component.
