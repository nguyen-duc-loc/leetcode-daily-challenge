class Solution {
   public:
    // Same as 3305. Count of Substrings Containing Every Vowel and K Consonants
    // I
    long long countOfSubstrings(string word, int k) {
        return substringsWithAtMost(word, k) -
               substringsWithAtMost(word, k - 1);
    }
   ...
   private:
    ...
    long substringsWithAtMost(const string& word, int k) {
        ...
        for (int l = 0, r = 0; r < word.length(); ++r) {
            ...
            if (uniqueVowels == 5)
                // Add substrings containing every vowel with at most k
                // consonants to the answer. They are word[l..r], word[l
                // + 1..r], ..., word[min(vowelLastSeen[vowel])..r]
                res += min({vowelLastSeen['a'], vowelLastSeen['e'],
